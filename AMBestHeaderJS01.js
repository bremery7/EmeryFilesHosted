...
(function () {
    ...
    /* ── Init ────────────────────────────────────────────────────── */
    /*
        MediaSpace is a React SPA — by the time this JS file runs
        the body is present but #outerWrap may still be rendering.
        We use a short polling loop to wait for it rather than a
        simple DOMContentLoaded, which fires before React mounts.
    */
    function waitForOuterWrap(attempts) {
        attempts = attempts || 0;
        if (attempts > 40) {
            // Gave up after ~2s — fall back to body insertion
            injectHeader();
            return;
        }
        if (document.getElementById('outerWrap')) {
            injectHeader();
        } else {
            setTimeout(function () {
                waitForOuterWrap(attempts + 1);
            }, 50);
        }
    }
