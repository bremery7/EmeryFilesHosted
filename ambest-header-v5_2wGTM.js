...
=======================================================================
  ...
  What this file does:
    1. Immediately inserts the GTM <noscript> iframe as the first
       element in <body> (required for cookie consent load order)
    2. Waits for the MediaSpace DOM to be ready
    3. Finds #outerWrap (the top-level MediaSpace content wrapper)
    4. Inserts the AM Best header HTML immediately before it
    5. Wires up mobile toggle + mobile dropdown click behavior

  The AM Best header stacks ABOVE the native MediaSpace header.
  MediaSpace's own header (#js-react-header) is left untouched.

  NOTE: The bodyJS field in KMS admin should be cleared — GTM noscript
  injection is now handled here to guarantee correct load order.
...
(function () {
    ...
    /*
        This MUST run before anything else in <body>. By injecting it
        here — at the top of this script which itself loads via
        headerJSlinks — we guarantee the GTM noscript iframe is the
        first element inserted into <body>, satisfying cookie consent
        platform load order requirements.

        The data-gtm attribute acts as a guard to prevent duplicate
        insertion if this function is ever called more than once.
    */
    function injectGTMNoscript() {
        if (document.querySelector('noscript[data-gtm]')) return;

        var ns = document.createElement('noscript');
        ns.setAttribute('data-gtm', 'true');

        var iframe = document.createElement('iframe');
        iframe.src = 'https://www.googletagmanager.com/ns.html?id=GTM-PQN2Z4Q';
        iframe.height = '0';
        iframe.width = '0';
    ...
    /* ── Interactivity — click to open on ALL screen sizes ──────── */
    function wireInteractivity(header) {
        ...
        function closeAllDropdowns() {
            // Only close top-level menu items, not the mobile nav toggle
            header.querySelectorAll('.amb-header__menu > li.is-open').forEach(function (el) {
                el.classList.remove('is-open');
            });
        }

        /* Top-level dropdowns — click to open/close on ALL screen sizes */
        header.querySelectorAll('.amb-header__menu > li > a').forEach(function (link) {
            link.addEventListener('click', function (e) {
                ...
                if (dropdown) {
                    e.preventDefault();
                    var isAlreadyOpen = parent.classList.contains('is-open');
        ...
        /* Second-level submenus — hover on desktop, click on mobile */
        ...
        /* Third-level submenus — hover on desktop, click on mobile */
    ...
    /*
        ...
        Phase 1 — Immediate: inject the AM Best header at the top of <body>
        the moment this script runs, then inject the GTM noscript after it.
        Because GTM noscript uses insertBefore(firstChild), it lands ABOVE
        the header — making it the true first element in <body>. The user
        sees the dark blue bar instantly while MediaSpace's React app is
        still mounting.
        ...
        GTM noscript is NOT repositioned — it stays as the first element
        in <body> permanently, as required by cookie consent platforms.
    */

    function injectImmediate() {
        // Step 1: Inject AM Best header (don't inject twice)
        if (document.querySelector('.amb-header')) return;
        ...
        // Step 2: GTM noscript injected AFTER header so insertBefore(ns, firstChild)
        // places it above the header — making it the true first element in <body>
        injectGTMNoscript();
